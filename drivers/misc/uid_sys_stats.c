...
static inline int trylock_pid(pid_t pid)
{
	return spin_trylock(
		&pid_lock[hash_min(pid, HASH_BITS(app_hash_table))]);
}
...
static inline void lock_pid(pid_t pid)
{
	spin_lock(&pid_lock[hash_min(pid, HASH_BITS(app_hash_table))]);
}
...
static inline void unlock_pid(pid_t pid)
{
	spin_unlock(&pid_lock[hash_min(pid, HASH_BITS(app_hash_table))]);
}
...
static int sys_app_cputime_show(struct seq_file *m, void *v)
{
	...
	for (bkt = 0, pid_entry = NULL; pid_entry == NULL &&
		bkt < HASH_SIZE(app_hash_table); bkt++) {
		lock_pid_by_bkt(bkt);
		hlist_for_each_entry(pid_entry, &app_hash_table[bkt], hash) {
...
static const struct proc_ops sys_app_cputime_fops = {
	.proc_open		= sys_app_cputime_open,
	.proc_read		= seq_read,
	.proc_lseek		= seq_lseek,
	.proc_release	= single_release,
};
...
static void init_app_hash_table_and_lock(void)
{
	int i;
